...
int main() {
    ...
    LogLevel selectedLevel = (logLevelChoice == 2) ? LogLevel::WARN
                             : (logLevelChoice == 3) ? LogLevel::ERROR
                             : (logLevelChoice == 4) ? LogLevel::DEBUG
                             : LogLevel::INFO;

    LoggerManager::configure(choice == 1, choice == 2, selectedLevel);
    Logger& logger = LoggerManager::getInstance();

    logger.log(LogLevel::INFO, "Starting Task Manager...");
    logger.log(LogLevel::DEBUG, "Logger successfully configured.");
